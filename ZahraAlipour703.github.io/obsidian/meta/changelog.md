...
- **Built out the home view — Zahra Alipour portfolio.** Implemented the previously
  empty `views/home.tsx` as a full one-page CV/AI-research portfolio (Hero, About,
  Research, Projects, Publications, Experience, Contact), ported from a prior
  static site. Design: dark navy palette (`--background: #0a0e14`) with a cyan-teal
  accent (`--accent: #3fe0c9`) and a bespoke "detection overlay" motif —
  `DetectionLabel` (YOLO-style class-name eyebrows) and `DetectionFrame`
  (spring-driven bounding-box corner brackets on hover, built on `<Hover>` +
  `trigger` refs) — new primitives in `components/ui/`. Added `JetBrains_Mono` next
  to `Onest` for the mono/label voice. Content lives in `src/data/mocks/home.ts`
  per [[component-conventions]] (no hardcoded copy in components). `siteConfig` in
  `lib/site.ts` updated with real SEO metadata.
  - **Gotcha found & fixed:** `spring-text-engine`'s `TextEngine` has no internal
    `"use client"` boundary, so importing it into a Server Component (`about.tsx` →
    `SectionHeading`) crashed RSC module evaluation (`createContext is not a
    function`). Fixed by marking `SectionHeading` and `Hero` (both leaves that
    render `TextEngine` directly) as `"use client"` — consistent with hard rule #7.
  - Verified: `yarn lint` clean, `tsc --noEmit` clean, `yarn dev` renders the real
    content at `GET / 200`. `yarn build` could not be verified in the sandbox this
    was built in (Google Fonts fetch blocked by network egress rules there) — not
    a code issue, expected to build normally with standard network access.
...
- **Configured for static export + GitHub Pages.** Added `output: "export"` and
  `images.unoptimized: true` to `next.config.ts` (no server on GitHub Pages — no
  ISR, no API routes, no `next/image` optimization). Removed the starter's
  example `app/api/contact/route.ts` and the now-unused `lib/api/` +
  `lib/api-client.ts` (not compatible with static export, and unused by the
  home view — contact is plain `mailto:`/social links). Simplified `env.ts` to
  drop the server-only `CONTACT_ENDPOINT` schema accordingly. Added
  `.github/workflows/deploy.yml` — builds with `yarn build` and deploys `out/`
  to GitHub Pages via `actions/deploy-pages`. `NEXT_PUBLIC_SITE_URL` set to
  `https://zahraalipour703.github.io` (a user-page repo, served at the domain
  root — no `basePath` needed).
